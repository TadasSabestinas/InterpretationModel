...
function app() {
  return {
    ...
    async tryCompare() {
      ...
      const notXml = [this.cmpFileA, this.cmpFileB].find(
    ...
    // sorted ascending so worst packages appear first makes the most critical ones easy to spot
    sortedPackages() {
      return [...(this.report?.packages || [])].sort(
    ...
    currentClass() {
      ...
      return this.report.classes.find(
        (c) => c.package_name === pkg && c.class_name === cls,
      );
    },
      ...
      return [
        {
          key: "mean",
          label: "Mean(L,B)",
          value: c.mean_line_branch,
          unit: "%",
        },
        {
          key: "wlc",
          label: "Meth. mean",
          value: c.mean_method_cov,
          unit: "%",
        },
      ];
    },
    ...
    compareMetrics() {
      ...
      return [
        ...
        { key: "method", label: "Method %", a: a.method_pct, b: b.method_pct },
        {
          key: "mean",
          label: "Mean(L,B)",
          a: a.mean_line_branch,
          b: b.mean_line_branch,
        },
        {
          key: "geo",
          label: "Geo mean",
          a: a.coverage_geo_mean,
          b: b.coverage_geo_mean,
        },
        {
          key: "wlc",
          label: "Meth. mean",
    ...
    _buildComparedPackages() {
      ...
      const pkgsA = new Map(
        this.comparison.a.packages.map((p) => [p.package_name, p]),
      );
      const pkgsB = new Map(
        this.comparison.b.packages.map((p) => [p.package_name, p]),
      );
      const allNames = new Set([...pkgsA.keys(), ...pkgsB.keys()]);
      const delta = (b, a, field) =>
        b && a ? +(b[field] - a[field]).toFixed(2) : null;
      return [...allNames]
        .map((name) => {
        ...
        .sort((x, y) => {
          if (x.delta_score !== null && y.delta_score !== null)
            return x.delta_score - y.delta_score;
    ...
    cmpMatchCount() {
      return (this.comparedPackages || []).filter((p) => p.status === "matched")
        .length;
    },
    ...
    async explainPackage() {
      const pkg = this.report.packages.find(
        (p) => p.package_name === this.drillPath.package,
      );
    ...
    async explainComparison() {
      ...
      await this._streamLlm("/api/explain/compare", {
        target_a: a,
        target_b: b,
        label_a: a.project_name,
        label_b: b.project_name,
        level: "project",
        show_prompt: true,
      });
    },
